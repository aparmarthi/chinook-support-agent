...
- **The cost of getting it wrong is real** — though not for the reason first written here. ⚠️ **Correction, found later:** the docs figures I used (30-40 LCUs to initialize, 10-15 per scan) don't match the [pricing calculator](https://www.langchain.com/pricing-langsmith), which scales per-run LCUs with trace volume: 2 at 1k traces, 8 at 10k, 30 at 100k, four runs a day. **The 30-LCU figure is the 100,000-trace case**, so for a 2,000-trace project the real number is ~3.8 LCUs (~$5.70) per run — about 8x lower than I'd written. The sequencing advice survives the correction and gets *stronger*: at four automatic runs a day, the expensive thing was never initialization, it's leaving it enabled (~$686/month at this volume).
- **Cost:** Two days of planning against a number that was wrong by 8x, and I came within one conversation of quoting it to an interviewer. Caught only by pulling the pricing page while checking something else.
- **This is the second time the same trap fired.** The OpenAI rates in the build plan were challenged four times as "5x too low" by sources cached before a price cut. Same shape: a plausible figure, a stale or mismatched source, and a decision about to be made on it. **Pricing for this category moves faster than the documents describing it**, which for a Deployed Engineer is not trivia — the finance question is what stalls a rollout after the technical evaluation has already succeeded.
- **Additional suggestion:** the docs page and the pricing calculator should agree, or the docs figure should state the trace volume it assumes. A flat "30-40 LCUs" reads as a constant when it's a point on a curve.
...
### [LangSmith] Engine's availability gate is org *type*, and nothing says so until you're in the settings page — **Papercut with a planning cost**
- **Expected:** From the pricing page, Engine is a Plus-plan feature — "Access to Deployment, Engine, and more" sits under the $39/seat tier. The reasonable inference is that access is a billing decision: pay $39, get Engine.
- **Actual:** On a personal organization the toggle is present but disabled, reading **"Engine is not available for personal organizations."** The gate is the *org type*, not the plan. Upgrading a personal org would not have unlocked it; you need a non-personal organization, which is a different object with a different setup path.
- **The confusing part is that the API says otherwise.** `GET /api/v1/orgs/current` returns `engine_default_enabled: true` on this org, alongside `plan_tier: "free"` and `engine_lcu_spend_limit_monthly: 0.0`. Reading the config programmatically — which is what you'd do to gate a demo or a CI job — tells you Engine is on when the product will refuse it.
- **Cost:** Three days of planning treated Engine as conditional-on-budget rather than unavailable-by-construction, including a Day 3 task to seed 150 conversations whose only purpose was to give Engine something to analyze. That task is now cut. The Slack question asked about *plan* access and about who pays for LCUs; neither would have surfaced this, because I didn't know org type was the axis.
- **Suggestion:** Two things. **(1)** Say it on the pricing page — "Engine requires a non-personal organization" belongs next to the plan comparison, because that's where the buying decision is made and $39 doesn't fix it. **(2)** Make `engine_default_enabled` reflect entitlement, or add an `engine_available` field. A config flag that reports `true` for something the UI blocks is a trap for exactly the automated setup path the flag exists to serve.
- ✅ **Demo value: moderate, and better in Q&A than in the friction block.** It's a real finding with a clean suggestion, but "I couldn't access a feature" is thinner than the three findings that came out of actually building. Keep it as the answer to "why no Engine?"
...
- **The workaround does not fully close it, which is its own small finding:** the server mints one assistant per graph with empty context, and that one still throws. It cannot be removed — deleting it only lasts until the next `langgraph dev` restart recreates it — and in Studio's dropdown it sorts *above* the two identities that work. So the default entry for a graph whose identity is required is the entry that crashes. `setup_studio_assistants.py` now patches it to carry customer 6 and renames it "chinook_support (defaults to Helena)", which turns a misclick into the wrong tenant rather than a traceback. That is a demo safeguard, not a fix; a graph with a required context field arguably should not get an auto-assistant that cannot satisfy it.
...
### [Docs/OSS] The pre-graph authorization layer is real, and I built a wrapper because I never found it — **Docs gap, and the most expensive one here**
- **Versions:** `langgraph-api` 0.11.2, `langgraph-sdk` 0.4.2, `langgraph dev`.
- **What I was doing:** Enforcing that a thread belongs to exactly one customer, so Helena's conversation cannot be resumed as Richard. I had already established the hard part correctly — graph middleware is too late, because by the time `before_agent` runs the checkpoint is loaded and you are guarding a door someone walked through.
- **What I concluded, and it was half wrong:** that the check therefore had to live *outside* the server, in an application wrapper. "Middleware is too late" rules out graph code. It says nothing about the Agent Server, which has `@auth.authenticate` and `@auth.on.threads.*` running before a run is created — exactly the ordering I wanted. I wrote `SupportGateway` instead. Correct, and unnecessary.
- **Why I missed it:** the material I was living in — `create_agent`, middleware, `ToolRuntime`, context schemas — is the *graph-authoring* surface, and thread authorization is not part of it. Custom auth is documented thoroughly, under deployment. Nothing in the middleware documentation says "if you are reaching for `before_agent` to do authorization, you want a different layer," which is the one cross-reference that would have saved the detour. The two surfaces read as separate products, and the question "where does authorization go" has an answer in only one of them.
- **What it actually does, measured** (`tests/test_native_auth.py`, live server): 401 unauthenticated; the server stamps `metadata.owner` rather than trusting the caller; a cross-tenant read returns **404 rather than 403**, so the error cannot confirm the thread exists; a cross-tenant run returns 404 with **zero runs created**. Deleting the `auth` key turns nine of ten red.
- **The genuinely good part — and this is the best security DX in the stack:** with handlers registered on `threads` only, the server refuses to be quietly insecure. At startup it logs that `assistants`, `crons`, and `store` have no authorization handler, names the consequence — *"a common source of cross-user data leaks"* — and prints the default-deny snippet to fix it. I had not thought about `store` at all. Most frameworks let unprotected routes be silent; this one treats an unhandled resource as a finding.
- **The catch that decided the architecture:** Studio is exempt from custom auth by default and authenticates the *developer* — `ctx.user` is a `StudioUser`, so filtering by `ctx.user.identity` binds demo threads to whoever opened the browser. `disable_studio_auth: true` closes the exemption and returns 401 to Studio itself, which has no way to send a bearer token. There is no configuration where Studio both authenticates as a customer and stays usable. So the gateway stays for the Studio path and the server owns every credentialed caller (ADR-022).
- **Second-order cost:** authentication applies to *every* route, so the assistant-provisioning script started failing 401 and needed an operator credential. Obvious in hindsight; not obvious while wondering why setup broke.
- **Cost:** ~90 minutes to implement, probe, and mutation-test — against roughly half a day already spent building the wrapper.
- **Suggestion:** one line in the middleware docs pointing from `before_agent` to resource authorization, and one sentence in the custom-auth guide saying plainly that Studio is exempt by default and what that means for anyone whose tenant is not the developer. Both are discoverability fixes, not behavior changes.
- **Demo value: high, and it is a reversal rather than a complaint.** It answers *"why didn't you just use `@auth.on.threads`?"* before it is asked, the startup warning is a genuine compliment with evidence behind it, and the reason the wrapper survived is a real finding about Studio rather than an excuse for keeping code.

### [OSS] The create_run authorization payload nests the run's own arguments, so the natural handler enforces nothing — **Sharp edge, and the most dangerous kind: it fails open silently**
- **Versions:** `langgraph-api` 0.11.2, `langgraph-sdk` 0.4.2.
- **What I was doing:** binding runtime identity to the credential inside `@auth.on.threads.create_run`, so a caller cannot pass someone else's `customer_id` in run context.
- **What I expected:** `value` is the create-run request, so `value["context"]` and `value["config"]["configurable"]` are where the caller's context lives. That is what the request body looks like and what the SDK signature suggests.
- **What is actually there:** the top level holds routing fields — `thread_id`, `assistant_id`, `run_id`, `status`, `metadata`, `multitask_strategy` — and every argument belonging to the run is one level down under `kwargs`: `value["kwargs"]["context"]`, `value["kwargs"]["config"]["configurable"]`. The documented `metadata` mutation works, because `metadata` *is* top level, and that is exactly what makes the mistake stick: the one field the examples show you mutating is the one field where the obvious guess is right.
- **Why it is worse than a papercut:** the wrong version does not raise, log, or fail a test. `value.get("context")` returns `None`, the loop over claimed identities runs zero times, and the handler returns success. I wrote it, restarted the server, re-ran the exploit, and **got a 200 with the other customer's data** — with a handler that read as correct in review. An authorization bug whose failure mode is "silently allows everything" should not be reachable by reading the field name you would expect.
- **How I found it:** logging `sorted(value.keys())` from inside the handler. That should not be the discovery mechanism for the shape of an authorization payload.
- **Bonus finding, and a genuinely nice one:** `configurable` already contains `langgraph_auth_user`, `langgraph_auth_user_id`, and `langgraph_auth_permissions`, injected by the server. The authenticated identity is available to the graph without any of this — which is the cleaner production answer and is not signposted anywhere I looked while designing the context schema.
- **Cost:** ~40 minutes, most of it after the fix "worked", spent confused about why a handler that was demonstrably running was demonstrably not enforcing.
- **Suggestion:** type the `create_run` value so `kwargs` is visible, or surface `context` and `config` at the top level of the authorization payload where callers reach for them. Failing both, one example in the custom-auth guide that validates run context rather than only stamping metadata — every example mutates `metadata`, which is the one case where the intuitive path is also the correct one.
- **Demo value: high, and it pairs with the previous entry rather than repeating it.** That one is "I didn't find the layer." This one is "I found it, used it, and it silently did nothing" — with a measured cross-tenant read as the evidence, and a mutation test that turns four assertions red to show the fix is real.
