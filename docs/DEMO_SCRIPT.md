...
> "Both arms resolved all thirty. The supervisor bought nothing, and it cost one-point-seven times the median latency — six-point-six seconds against three-point-eight — and sixty-one tool calls against forty-five. So flat ships.
>
> Here's the part I'd actually want you to take away. **A week ago this table said the supervisor won**, thirty against twenty-nine, and I shipped flat anyway — because I'd written the bars before the supervisor existed, and one example out of thirty was never going to clear a two-second latency budget. Then I changed the escalation prompt, reran both arms, and **the lead disappeared.** They tie.
>
> That margin was never a quality difference, and the only reason I didn't report it as one is that the decision rule was written down before I saw it. That's the whole case for pre-registration, and I'd rather show it to you happening than describe it."

> ✅ **This is now the strongest block in the deck — rehearse it verbatim.** The story used to be "the complicated thing won and I threw it away." It is now "the complicated thing appeared to win, I declined to bank it, and the next run proved me right." That is a demonstration of evaluation discipline rather than a claim of it, and it is the single most role-relevant sixty seconds you have. **Do not soften it into "the results were noisy."**

> ⚠️ **Never say "it flipped" or "run-to-run variance".** Three overclaims have been caught in this paragraph across drafts. The supportable framing is above: a controlled pair at `12cefb1` and a controlled pair at `a29a530`, and the resolution count did not separate the arms in either direction that survived a rerun. Runs before `12cefb1` carry **no recorded commit** and differ in code as well as sampling — they show sensitivity, and they are not evidence of anything else.

> ⚠️ **Have the provenance ready, don't volunteer it.** Both arms: commit `a29a530`, clean tree, dataset digest `6ba144216f8d`, evaluator digest `decda2664959`, `gpt-5.6-luna`. Frozen in [`reports/experiment.md`](../reports/experiment.md), generated from the result files rather than typed. `scripts/freeze_experiment.py` refuses to publish two arms that disagree on commit, model, dataset, or evaluators — which is the mistake that produces most bad benchmark tables. **The prior freeze at `12cefb1` is still on disk**, which is what lets you show the reversal rather than assert it.
...
| "This ran for about $12" | The measured figure. Every priced eval row on disk sums to **$0.22**, so the planning estimate overstates it by more than 50x — quoting the estimate would undersell the strongest cost number you have. |
| "The entire build cost under a dollar" | **Only if you pulled the OpenAI dashboard.** $0.22 is every *priced eval row*; it excludes Studio traffic and 230 rows recorded before token capture. Say the scoped number, which is checkable, rather than the round one, which isn't. |
...
| API spend on slide 2 | `$0.22`, summed from `evals/results/` | ✅ **resolved without the dashboard** |

> ✅ **The `$__.__` blank is gone, and the replacement is a better number.** Rather than a dashboard total you'd be quoting from memory, slide 2 now reads **$0.22 — every priced row in `evals/results/`**: 220 rows across 19 result files, both experiment arms, every rerun. It is summed from artifacts in the repo, so if someone asks where it comes from you can open the directory. Say it precisely: *"twenty-two cents of API spend across every eval run I have on disk."*
>
> ⚠️ **It is a floor, not a total** — it excludes Studio traffic, trace capture, and 230 recorded rows from before token capture existed. **So do not say "the entire build cost under a dollar" unless you pull the OpenAI dashboard first.** The bounded claim is stronger anyway, because it is checkable. If you do pull the dashboard before Thursday, add it as a second row rather than replacing this one.

> ⚠️ **One red row remains and it is the important one.** Everything generated from code is regenerable and current. The screenshots need a logged-in browser and are the offline fallback for Block 5, which is the section this whole demo is built to land.
...
- [ ] *Optional* — OpenAI dashboard total pulled and added to slide 2 as a second row. Slide 2 already carries the defensible `$0.22` summed from `evals/results/`; this only unlocks the broader "whole build" phrasing
